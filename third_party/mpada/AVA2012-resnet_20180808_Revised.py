...
if __name__ == '__main__':
    ...
    parser.add_argument('--crop_method_TR', help='The crop method during the training process [ShortestEdgeResize]',
                        type=str,
                        choices=['GoogleNetResize', 'ShortestEdgeResize', 'GlobalWarp', 'CAMCrop', 'CAMCropR'],
    ...
    assert (args.load is None) or (
            args.load_npy is None), '*** load and load_npy cannot be of Non-None at the same time ***'
    ...
    if (args.crop_method_TR == 'CAMCrop') or (args.crop_method_TR == 'CAMCropR'):
        assert os.path.isdir(
            args.CAM_dir_pkl), '*** THE CAM_dir_pkl DOES NOT EXIST FOR CAMCrop & CAMCropR TO PERFORM ***'
        CAM_DIR_PKL = args.CAM_dir_pkl

    # add @ 20171229: to check the value setting of JensenFactor
    if args.JensenFactor is not None:
    ...
    if args.CAM_viz is not None:
        batch = 128

        # @ 20171125
        if args.CAM_viz == 'train':
            assert args.crop_method_TR not in ['CAMCrop',
                                               'CAMCropR'], '*** CAMCrop or CAMCropR IS NOT ALLOWED IN CAM_viz ***'
        ...
        ds = get_data(args.CAM_viz, args.data, batch,
                      args.crop_method_TS if args.CAM_viz == 'val' else args.crop_method_TR, \
        ...
        logger.set_logger_dir('./train_log/AVA2012{6}-{0}-d{1}-{2}-{3}{4}{5}_LRT3'.format(args.mode, args.depth,
                                                                                          ...
                                                                                          args.crop_method_TS,
                                                                                          initial_strategy,
                                                                                          '' if args.JensenFactor == 0.0 else '_JE{}'.format(
                                                                                              args.JensenFactor),
                                                                                          '' if args.aesthetic_level == AESTHETIC_LEVEL else '-A{}'.format(
                                                                                              args.aesthetic_level)))
